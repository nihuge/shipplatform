...
<!--        <link rel="stylesheet" href="/Public/bootstrapBox/css/bootstrap.css" type="text/css"/>-->
            <link rel="stylesheet" href="/Public/bootstrapBox/css/awesome-bootstrap-checkbox.css" type="text/css"/>
            <link rel="stylesheet" href="/Public/bootstrapBox/Font-Awesome/css/font-awesome.min.css" type="text/css"/>

    <!--    <link rel="stylesheet" href="/Public/bootstrapBox/css/default.css" type="text/css"/>-->
...
<div class="main-container ace-save-state" id="main-container">
    ...
    <div class="page-container">
        <div>
            <form action="/admin.php?s=/User/index" method="get">
        ...
        <table id="sample-table-1" class="table table-striped table-bordered table-hover">
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr align='center'>
                    ...
                    <?php if($v['status'] == 2): ?><td><a href="javascript:;" onclick="changestatus('<?php echo ($v['id']); ?>','1')" style="color:red">解除冻结</a>
                        </td>
                        <?php else: ?>
                        <td><a href="javascript:;" onclick="changestatus('<?php echo ($v['id']); ?>','2')" style="color:black">冻结用户</a>
                        </td><?php endif; ?>
                    <td>
                        <a href="<?php echo U('User/edit',array('firmid'=>$v['firmid'],'id'=>$v['id']));?>">修改</a>

                        |&nbsp;
                        <a href="<?php echo U('User/configSearch',array('id'=>$v['id']));?>">分配查询权限</a>
                        <!-- 只有公司管理才可以添加操作人员和复制给操作员权限 -->
                        <?php if($v['pid'] == '0'): ?>|&nbsp;<a href="#" onclick="copyJur(<?php echo ($v['id']); ?>)">复制权限到</a>
                            |&nbsp;<a href="<?php echo U('User/add',array('firmid'=>$v['firmid'],'id'=>$v['id']));?>">新增操作员</a><?php endif; ?>
                    </td>
    ...
    <script>
        ...
        function copyJur(id) {
            ...
            $.ajax({
                url: "<?php echo U('User/copyJur');?>/id/" + id,
                type: "GET",
                dataType: "json",
                success: function (data) {
                    var operator = data;
        ...
        function changestatus(id, status) {
            layer.confirm('您确定要修改状态吗？', {
                ...
                $.ajax({
                    url: "/admin.php?s=/User/changestatus",
                    type: "POST",
                    data: {
                        "id": id,
                        "status": status
