...
<div class="main-container ace-save-state" id="main-container">
    ...
    <div class="page-container">
		...
		<div>
			...
			<form action="/admin.php?s=/Firm/index" method="get">
		...
		<br/>
        <div>
            <a href="<?php echo U('Firm/add');?>" class="btn btn-primary">新增</a>
        </div>
        <h4></h4>
        <table id="sample-table-1" class="table table-striped table-bordered table-hover">
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr align='center'>
                    ...
                    <td>
                        <?php if($v['membertype'] == '1'): ?>会员费
                            <?php elseif($v['membertype'] == '2'): ?>
                            服务费<?php endif; ?>
                    </td>
                    ...
                    <td><?php echo ($v['creditline']); ?></td>
                    <td>
                        <a href="<?php echo U('Firm/edit',array('id'=>$v['id']));?>">查看/修改</a>&nbsp;|
                        <a href="<?php echo U('Firm/adminmsg',array('id'=>$v['id']));?>">管理员</a>&nbsp;|
                        <?php if($v['firmtype'] == '检验'): ?><a href="<?php echo U('Firm/configSearch',array('id'=>$v['id'],'firmtype'=>$v['firmtype']));?>">查询权限</a>&nbsp;|<?php endif; ?>
                        <a href="<?php echo U('Firm/configOperator',array('id'=>$v['id'],'firmtype'=>$v['firmtype']));?>">操作权限</a>&nbsp;|
                        <a href="<?php echo U('Firm/addpersonality',array('id'=>$v['id']));?>"> 个性化字段 </a>&nbsp;|
                        <?php if($v['del_sign'] == 1): ?><a href="javascript:;" onclick="delFirm(<?php echo ($v['id']); ?>)" style="color:#3ca0fe">删除</a>
                            <?php else: ?>
                            <a href="javascript:;" onclick="recoverFirm(<?php echo ($v['id']); ?>)" style="color:#3ca0fe">恢复</a>
                            <?php if($adminmsg['group_id'] == 1): ?>&nbsp;|<a href="javascript:;" onclick="relDelFirm(<?php echo ($v['id']); ?>)"
                                          style="color:#3ca0fe">彻底删除</a><?php endif; endif; ?>
                    </td>
        ...
        function recoverFirm(firmid) {
            layer.confirm('您确定要恢复公司吗？', {
                ...
                $.ajax({
                    url: "<?php echo U('Firm/recoverFirm');?>",
                    type: "POST",
                    data: {
                        firmid: firmid
                    },
        ...
        <?php if($adminmsg['group_id'] == 1): ?><script type="text/javascript">
                function relDelFirm(firmid) {
                    layer.confirm('危险操作，此操作无法撤回数据。您确定要彻底删除公司吗？', {
                        ...
                        $.ajax({
                            url: "<?php echo U('Firm/relDelFirm');?>",
                            type: "POST",
                            data: {
                                firmid: firmid
                            },
