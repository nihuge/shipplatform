...
<div class="main-container ace-save-state" id="main-container">
	...
	<div class="page-container">
        <div>
            <a href="<?php echo U('Cabin/add');?>" class="btn btn-primary">新增</a>
        </div>
        <h4></h4>
        <div>
            <form action="/admin.php?s=/Cabin/index" method="get">
            	<input type="hidden" name="tname" value="<?php echo ($tname); ?>">
            	<input type="hidden" name="c" value="Cabin">
            	<input type="hidden" name="a" value="index">
            	船名：<select name="shipid">
            	...
            	<input type="submit"  value="筛选" style="height:28px;width:80px">
            </form>
        </div>
        <h4></h4>
		<table id="sample-table-1" class="table table-striped table-bordered table-hover">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr align='center'>
					...
					<td><?php echo ($v['cabinname']); ?></td>
					<td><?php echo ($v['altitudeheight']); ?></td>
                    <td><?php echo ($v['bottom_volume']); ?></td>
					<!-- <td><?php echo ($v['bottom_volume_di']); ?></td> -->
					<td><?php echo ($v['pipe_line']); ?></td>
					<td>
                        <a href="<?php echo U('Cabin/edit',array('id'=>$v['id']));?>">修改</a>
                    </td>
                    <td>
                    	<?php if($v['tankcapacityshipid'] != null): ?><a href="<?php echo U('Tripbystern/zong',array('tname'=>$v['tankcapacityshipid'],'cabinid'=>$v['id'],'shipid'=>$v['shipid']));?>">容量管理</a><?php endif; ?>
                        <?php if($v['rongliang'] != null): ?><a href="<?php echo U('Rong/rong',array('tname'=>$v['rongliang'],'cabinid'=>$v['id'],'shipid'=>$v['shipid']));?>">容量(容量)管理</a>&nbsp;/&nbsp;<?php endif; ?>

                        <?php if($v['zx'] != null): ?><a href="<?php echo U('Zongxiu/zx',array('tname'=>$v['zx'],'cabinid'=>$v['id'],'cabinname'=>$v['cabinname'],'shipid'=>$v['shipid']));?>">纵修(容量)管理</a><?php endif; ?>
                        <?php if($v['rongliang_1'] != null): ?>&nbsp;|&nbsp;<a href="<?php echo U('Rong/rong',array('tname'=>$v['rongliang_1'],'cabinid'=>$v['id'],'shipid'=>$v['shipid']));?>">容量(底量)管理</a>&nbsp;/&nbsp;<?php endif; ?>
                        <?php if($v['zx_1'] != null): ?><a href="<?php echo U('Zongxiu/zx',array('tname'=>$v['zx_1'],'cabinid'=>$v['id'],'shipid'=>$v['shipid']));?>">纵修(底量)管理</a><?php endif; ?>
                    </td>
				</tr><?php endforeach; endif; else: echo "" ;endif; ?>
			<tr>
				<td colspan=7 class="pages"><?php echo ($page); ?></td>
			</tr>
		</table>
	</div>
