...
<div class="main-container ace-save-state" id="main-container">
	...
	<div class="page-container">
        <div>
            <a href="<?php echo U('Admin/add');?>" class="btn btn-primary">新增</a>
        </div>
        <h4></h4>
		<table id="sample-table-1" class="table table-striped table-bordered table-hover">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr align='center'>
                    ...
                    <?php else: ?>
                        <td>
                            <?php if($v['title'] !== 'admin'): ?><a href="javascript:;" onclick="changestatus('<?php echo ($v['id']); ?>','2')" style="color:black">冻结用户</a><?php endif; ?>
                        </td><?php endif; ?>
					<td>
                        <a href="<?php echo U('Admin/edit',array('id'=>$v['id']));?>">修改</a>
                        <?php if($v['title'] !== 'admin'): ?>&nbsp;|&nbsp;<a href="javascript:;" onclick="del('<?php echo ($v['id']); ?>')" >删除</a><?php endif; ?>
                    </td>
...
function resetpwd(id) {
    layer.confirm('您确定要重置密码吗？', {
        ...
        $.ajax({
            url: "/admin.php?s=/Admin/resetpwd",
            type: "POST",
            data: {
                "id": id
            },
...
function changestatus(id, status) {
    layer.confirm('您确定要修改状态吗？', {
        ...
        $.ajax({
            url: "/admin.php?s=/Admin/changestatus",
            type: "POST",
            data: {
                "id": id,
                "status": status
