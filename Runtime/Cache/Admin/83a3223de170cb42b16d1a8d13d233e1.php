...
<div class="main-container ace-save-state" id="main-container">
    ...
    <div class="page-container">
        <div>
            <form action="/admin.php?s=/Search/index" method="get">
        ...
        <table id="sample-table-1" class="table table-striped table-bordered table-hover">
            ...
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr align='center'>
                    ...
                    <td><?php echo ($v['personality']['goodsname']); ?></td>
                    ...
                    <td><a href="/admin.php?s=/Search/msg/resultid/<?php echo ($v['id']); ?>" style="color:#3ca0fe">查看</a>&nbsp;/&nbsp;
                        <?php if($v['del_sign'] == 1): ?><a href="javascript:;" onclick="delWork(<?php echo ($v['id']); ?>)" style="color:#3ca0fe">删除</a>
                    <?php else: ?>
                    <a href="javascript:;" onclick="recoverWork(<?php echo ($v['id']); ?>)" style="color:#3ca0fe">恢复</a>
                    <?php if($adminmsg['group_id'] == 1): ?>&nbsp;/&nbsp;<a href="javascript:;" onclick="relDelWork(<?php echo ($v['id']); ?>)"
    ...
    <script type="text/javascript">
        $("#shipid").bigAutocomplete({
            ...
            data: [
            <?php
 foreach($shiplist as $l){ echo '{title:"'.$l['shipname'].'",show:"'.$l['shipname'].'"},'; } ?>
        ],
        ...
        $("select[name=firmid]").change(function () {
            ...
            if (id == '') {
                ...
                $.ajax({
                    url: "<?php echo U('Search/getFirmShip');?>",
                    data: 'firmid=' + id,
                    type: 'post',
                    async: false,
                    dataType: 'html',
        ...
        function delWork(resultid) {
            layer.confirm('您确定要删除作业吗？', {
                ...
                $.ajax({
                    url: "<?php echo U('Search/del_work');?>",
                    type: "POST",
                    data: {
                        resultid: resultid
                    },
        ...
        function recoverWork(resultid) {
            layer.confirm('您确定要恢复作业吗？', {
                ...
                $.ajax({
                    url: "<?php echo U('Search/recoverWork');?>",
                    type: "POST",
                    data: {
                        resultid: resultid
                    },
    ...
    <?php if($adminmsg['group_id'] == 1): ?><script type="text/javascript">
            function relDelWork(resultid) {
                layer.confirm('危险操作，此操作无法撤回数据。您确定要彻底删除作业吗？', {
                    ...
                    $.ajax({
                        url: "<?php echo U('Search/relDelWork');?>",
                        type: "POST",
                        data: {
                            resultid: resultid
                        },
