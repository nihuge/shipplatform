...
<div class="main-container ace-save-state" id="main-container">
    ...
    <div class="page-container">
        <div class="col-sm-12">
            <div id="dropdown14" class="tab-pane" style="overflow-x: scroll;height:720px;">
                <table id="sample-table-3" class="table table-striped table-bordered table-hover" style='margin:auto auto;text-align: center;'>
                    <form action="/admin.php?s=/User/configSearch" method="post">
                    <input type="hidden" name="id" value="<?php echo ($data['id']); ?>">
                    <tr>
                        <td colspan="3" style='color:red;text-align:left;'>
                            1：船舶不勾选默认查看操作员自己的作业记录；<br>
                            2：船舶不勾选，查看公司数据选项无效；<br>
                            3：船舶勾选，查询结果根据船舶选项跟查看公司选项筛选；
                        </td>
                    </tr>
                    <tr>
                        <td>查看公司数据</td>
                        <td colspan="2" style="text-align:left;">
                            <label>
                                <input type="radio" name='look_other' value="1" <?php echo ($data['look_other'] == '1') ? 'checked' : '';?>>被勾选的船舶，只看本公司数据
                            </label><br>
                            <label>
                                <input type="radio" name='look_other' value="2" <?php echo ($data['look_other'] == '2') ? 'checked' : '';?>>被勾选的船舶，可以看所有公司的数据
                            </label>
                        </td>
                    </tr>
                    
                    <?php if(is_array($firmlist)): foreach($firmlist as $key=>$v): ?><tr class="b-group">
                            <!--公司列表-->
                            <td style="width:200px;padding:auto auto;vertical-align: middle;font-size: 18px;">
                                <?php echo ($v['firmname']); ?>
                            </td>
                            <td style="width:70px;padding:auto auto;vertical-align: middle;font-size: 16px;">
                                <!-- <label for="firm<?php echo ($v['id']); ?>" style="vertical-align:middle;">全选</label>
                                <input type="checkbox" value="<?php echo ($value['id']); ?>"  style="vertical-align:middle;margin:0px 0px;" id="firm<?php echo ($v['id']); ?>" > -->
                                <label>
                                    <input class="ace ace-checkbox-2" type="checkbox" onclick='checkAll(this)'/>
                            ...
                            <td>
                                <table id="sample-table-1" class="table table-striped table-bordered table-hover" style='margin:auto auto;text-align: center;margin-bottom:30px;'>
                                    <tr>
                                        <td>
                                            <!--船列表-->
                                            <?php if(is_array($v['shiplist'])): foreach($v['shiplist'] as $key=>$value): ?><!-- <label for="ship<?php echo ($value['id']); ?>" style="vertical-align:middle"><?php echo ($value['shipname']); ?></label>
                                                <input type="checkbox" name="search_jur[]" value="<?php echo ($value['id']); ?>" <?php echo in_array($value['id'],$data['search_jur']) ? 'checked':'';?> style="vertical-align:middle;margin:0px 0px; " id="ship<?php echo ($value['id']); ?>"> -->
                                                <label>
                                                    <input class="ace ace-checkbox-2" type="checkbox" value="<?php echo ($value['id']); ?>" name="search_jur[]" id="ship<?php echo ($value['id']); ?>"
                                                    <?php echo in_array($value['id'],$data['search_jur']) ? 'checked':'';?>
                                                    />
                                                    <span class="lbl"> <?php echo ($value['shipname']); ?></span> 
                                                </label>
