        ...
        <div class="col-sm-12">
            <div id="dropdown14" class="tab-pane" style="overflow-x: scroll;height:720px;">
                <table id="sample-table-3" class="table table-striped table-bordered table-hover" style='margin:auto auto;text-align: center;'>
                    <form action="/admin.php?s=/Firm/configOperator" method="post">
                    <input type="hidden" name="id" value="<?php echo ($data['id']); ?>">
                    <?php if(is_array($firmShipList)): foreach($firmShipList as $key=>$v): ?><tr class="b-group">
                            <!--公司列表-->
                            <td style="width:200px;padding:auto auto;vertical-align: middle;font-size: 18px;">
                               <label>
                                    <input class="ace ace-checkbox-2" name="firm_jur[]" type="checkbox" onclick='checkAll(this)' value="<?php echo ($v['id']); ?>" <?php echo in_array($v['id'],$data['firm_jur']) ? 'checked':'';?>/>
                                    <span class="lbl">  <?php echo ($v['firmname']); ?></span> 
                                </label>
                            </td>
                            <td>
                                <table id="sample-table-1" class="table table-striped table-bordered table-hover" style='margin:auto auto;text-align: center;margin-bottom:30px;'>
                                    <tr>
                                        <td>
                                            <!--船列表-->
                                            <?php if(is_array($v['shiplist'])): foreach($v['shiplist'] as $key=>$value): ?><label>
                                                    <input class="ace ace-checkbox-2" type="checkbox"  name="operation_jur[]" value="<?php echo ($value['id']); ?>" <?php echo in_array($value['id'],$data['operation_jur']) ? 'checked':'';?>  onclick='checka(this)'/>
