    ...
    public function index()
    {
        if(intval(I('post.uid')) and I('post.imei') and intval(I('post.statue'))){
            ...
            if($msg1['code'] == 1){
                ...
                //成功
                $res =$notice->noticeList(intval(I('post.uid')),intval(I('post.statue')));
            }else{
                //用户验证失败
                $res = $msg1;
            }
        ...
        if(intval(I('post.uid')) and I('post.imei')){
            ...
            if($msg1['code'] == 1){
                ...
                $res = array(
                    ...
                    'list'=>$notice->noticeCount(intval(I('post.uid')))
                );
            }else{
                //用户验证失败
                $res = $msg1;
    ...
    public function allReaded()
    {
        if(intval(I('post.uid')) and I('post.imei')){
            ...
            if($msg1['code'] == 1){
                ...
                $res = array(
                    'code'=>$this->ERROR_CODE_COMMON['SUCCESS'],
                    'content'=>$notice->noticeReaded(intval(I('post.uid')))
                );
            }else{
                //用户验证失败
                $res = $msg1;
