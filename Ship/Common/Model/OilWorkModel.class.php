 ...
 *                      佛祖保佑       永无BUG
 * */
class OilWorkModel extends BaseModel
{
...
//            $union_array = json_decode($arrange_data['personality'],true);
    ...
    public function forntsearch($resultid)
    {
        ...
        $msg = $this
            ->field('f.forntleft,f.forntright,f.centerleft,f.centerright,f.afterleft,f.afterright,f.solt,r.qiansw,r.housw,r.houtemperature,r.houdensity,r.qiantemperature,r.qiandensity,f.resultid,r.id')
            ->alias('r')
    ...
    public function forntsearch1($resultid)
    {
        ...
        $msg = $this
            ->field('f.forntleft,f.forntright,f.centerleft,f.centerright,f.afterleft,f.afterright,f.solt,r.houtemperature,r.houdensity,r.qiantemperature,r.qiandensity,r.qiansw,r.housw,f.resultid')
            ->alias('r')
    ...
    public function forntOperation($datas)
    {
        ...
        if ($msg1['code'] == '1') {
            ...
            if ($datas['solt'] == '1') {
                ...
                //根据不同的货物计算各自货物的20摄氏度下的密度

                $this->process['density_process'] = array();
                //获取15摄氏度和20摄氏度下的视密度修正系数
                $cp15 = $this->getApparentDensityCorrectionFactor($datas['temperature'], 15);
                $this->process['density_process']['obTemperature'] = $datas['temperature'];
                $this->process['density_process']['cp15'] = $cp15;
                $cp20 = $this->getApparentDensityCorrectionFactor(15, 20);
                $this->process['density_process']['cp20'] = $cp20;
                //修正到实际密度
                $density = $datas['density'] * $cp15;
                $this->process['density_process']['density'] = $density;
                //获取15摄氏度下的密度
                $P15 = $this->iterativeCalculation($density, $datas['temperature'], $result_msg['oil_type'], $cp15);
                $this->process['density_process']['P15'] = $P15;

                $data1['houdensity'] = $P15;
...
//        $volume = corrent($midu, $data['temperature']);
        ...
        $this->process['density_process'] = array();
        //获取15摄氏度和20摄氏度下的视密度修正系数
        $cp15 = $this->getApparentDensityCorrectionFactor($data['ob_temperature'], 15);
        $this->process['density_process']['cp15'] = $cp15;
        $cp20 = $this->getApparentDensityCorrectionFactor(15, 20);
        $this->process['density_process']['cp20'] = $cp20;
        //修正到实际密度
        $density = $data['ob_density'] * $cp15;
        $this->process['density_process']['density'] = $density;
        //获取15摄氏度下的密度
        $P15 = $this->iterativeCalculation($density, $data['ob_temperature'], $msg['oil_type'], $cp15);
        $this->process['density_process']['P15'] = $P15;

        //转换成20摄氏度下的密度
        $P20 = $this->getP20($P15, $msg['oil_type']);
...
//        $this->process['VC']['formula'] = self::$function_process;
        ...
        $data['volume'] = $volume;
        $this->process['ob_temperature'] = $data['ob_temperature'];
        $this->process['ob_density'] = $data['ob_density'];
...
//        $this->process .= "table_contain_pipeline = " . $table_contain_pipeline . ", pipeline_stock:" . $pipeline_stock . " then:\r\n\tpipeline_volume=" . $gx . "\r\n";
        ...
        $water_ullage = $data['water_ullage'];        // 水空高
        $altitudeheight = round($data['altitudeheight'], 3);        // 基准高度

        $this->process['ullage'] = $ullage;
        $this->process['water_ullage'] = $data['water_ullage'];
        $this->process['water_sounding'] = $data['water_sounding'];
        ...
        switch ($shipmsg['suanfa']) {
            case 'a':
                ...
                if ($data['quantity'] == '2' and $altitudeheight == $ullage) {
                    //混合体积
                    $this->process['Mixed_volume'] = 0;
                    //水体积
                    $this->process['Water_volume'] = 0;
                    //货物体积
                    ...
                    //四种情况计算容量
                    $mixed_weight = round($this->suanfa($qiu, $ulist, $keys, $ullage, $chishui), 3);
                    $this->process['trim_table']['process'] = self::$function_process;
                    self::$function_process = array();
                    $this->process['trim_table']['SCV'] = $mixed_weight;
                    //混合体积
                    $this->process['Mixed_volume'] = $mixed_weight;

                    /**
                     * 然后计算水体积
                     */
                    //根据空高查询数据
                    $ulist = $this->downup($water_ullage, $shipmsg['tankcapacityshipid'], $field, $data['cabinid']);
                    $this->process['water_trim_table'] = array();
                    if (count($qiu) == '1' and count($ulist) == '1') {
                        $this->process['water_trim_table']['Ua'] = $ulist[0]['ullage'];
                        $this->process['water_trim_table']['Ub'] = $ulist[0]['ullage'];
                        $this->process['water_trim_table']['Da'] = $qiu[$keys[0]];
                        $this->process['water_trim_table']['Db'] = $qiu[$keys[0]];
                        $this->process['water_trim_table']['Caa'] = $ulist[0][$keys[0]];
                        $this->process['water_trim_table']['Cab'] = $ulist[0][$keys[0]];
                        $this->process['water_trim_table']['Cba'] = $ulist[0][$keys[0]];
                        $this->process['water_trim_table']['Cbb'] = $ulist[0][$keys[0]];
                    } elseif (count($qiu) == '2' and count($ulist) == '2') {
                        $this->process['water_trim_table']['Ua'] = $ulist[0]['ullage'];
                        $this->process['water_trim_table']['Ub'] = $ulist[1]['ullage'];
                        $this->process['water_trim_table']['Da'] = $qiu[$keys[0]];
                        $this->process['water_trim_table']['Db'] = $qiu[$keys[1]];
                        $this->process['water_trim_table']['Caa'] = $ulist[0][$keys[0]];
                        $this->process['water_trim_table']['Cab'] = $ulist[0][$keys[1]];
                        $this->process['water_trim_table']['Cba'] = $ulist[1][$keys[0]];
                        $this->process['water_trim_table']['Cbb'] = $ulist[1][$keys[1]];
                    } elseif (count($qiu) == '1' and count($ulist) == '2') {
                        $this->process['water_trim_table']['Ua'] = $ulist[0]['ullage'];
                        $this->process['water_trim_table']['Ub'] = $ulist[1]['ullage'];
                        $this->process['water_trim_table']['Da'] = $qiu[$keys[0]];
                        ...
                        $this->process['water_trim_table']['Cab'] = $ulist[0][$keys[0]];
                        $this->process['water_trim_table']['Cba'] = $ulist[1][$keys[0]];
                        $this->process['water_trim_table']['Cbb'] = $ulist[1][$keys[0]];
                    } elseif (count($qiu) == '2' and count($ulist) == '1') {
                        $this->process['water_trim_table']['Ua'] = $ulist[0]['ullage'];
                        $this->process['water_trim_table']['Ub'] = $ulist[0]['ullage'];;
                        $this->process['water_trim_table']['Da'] = $qiu[$keys[0]];
                        ...
                        $this->process['water_trim_table']['Cab'] = $ulist[0][$keys[1]];
                        $this->process['water_trim_table']['Cba'] = $ulist[0][$keys[0]];
                        $this->process['water_trim_table']['Cbb'] = $ulist[0][$keys[1]];
                    } else {
                        $this->process['water_trim_table']['Ua'] = "错误";
                        $this->process['water_trim_table']['Ub'] = "错误";
                        $this->process['water_trim_table']['Da'] = "错误";
                        $this->process['water_trim_table']['Db'] = "错误";
                        $this->process['water_trim_table']['Caa'] = "错误";
                        $this->process['water_trim_table']['Cab'] = "错误";
                        $this->process['water_trim_table']['Cba'] = "错误";
                        $this->process['water_trim_table']['Cbb'] = "错误";
                    }
                    //四种情况计算容量
                    $water_weight = round($this->suanfa($qiu, $ulist, $keys, $water_ullage, $chishui), 3);
                    $cabinweight = $mixed_weight - $water_weight + $gx;
                    $this->process['water_trim_table']['process'] = self::$function_process;
                    self::$function_process = array();
                    $this->process['water_trim_table']['SCV'] = $water_weight;
                    $this->process['Water_volume'] = $water_weight;
            ...
            case 'b':
                ...
                if ($data['quantity'] == '2' and $altitudeheight == $xiukong) {
                    ...
                    $mixed_weight = round($this->suanfa($qiu1, $ulist1, $keys1, $xiukong, $chishui), 3);
                     ...
                     */
//                wxy@csl-vastwin.com
                    $ulist = $this->downup($water_ullage, $shipmsg['zx'], $field, $data['cabinid']);
                    $this->process['water_trim_table'] = array();
                    if (count($qiu) == '1' and count($ulist) == '1') {
                        $this->process['water_trim_table']['Ua'] = $ulist[0]['ullage'];
                        $this->process['water_trim_table']['Ub'] = $ulist[0]['ullage'];
                    ...
                    $zongxiu1 = round($this->suanfa($qiu, $ulist, $keys, $water_ullage, $chishui), 0) / 1000;
                    $this->process['water_trim_table']['process'] = self::$function_process;
                    self::$function_process = array();
                    $this->process['water_trim_table']['TC'] = $zongxiu1;

                    //根据纵修与基准高度-空高的差值比较取小
                    $chazhi = round(($water_ullage - $data['altitudeheight']), 3);
                    if ($chazhi > $zongxiu1) {
                        $zongxiu = $chazhi;
                    } elseif ($chazhi < $zongxiu1) {
                        $zongxiu = $zongxiu1;
                    ...
                    $this->process['water_trim_table']['NowTC'] = $zongxiu;
                    $this->process['water_trim_table']['Sounding'] = $chazhi;

                    //得到修正空距 空距+纵倾修正值
                    $xiukong = round($water_ullage - $zongxiu, 3);
                    $this->process['Water_c_ullage'] = $xiukong;
                    ...
                    if ($data['quantity'] == '2' and $altitudeheight == $xiukong) {
                        ...
                        $keys1[] = 'capacity';  //容量表代表容量的字段
                        $qiu1 = array('capacity' => 1);    //随意定义，只要是一位数组
                        $this->process['water_capacity_table'] = array();
                        if (count($ulist1) == '1') {
                            $this->process['water_capacity_table']['U1'] = $ulist1[0]['ullage'];
                            $this->process['water_capacity_table']['U2'] = $ulist1[0]['ullage'];
                            $this->process['water_capacity_table']['CV1'] = $ulist1[0][$keys1[0]];
                            $this->process['water_capacity_table']['CV2'] = $ulist1[0][$keys1[0]];
                        } elseif (count($ulist1) == '2') {
                            $this->process['water_capacity_table']['U1'] = $ulist1[0]['ullage'];
                            $this->process['water_capacity_table']['CV1'] = $ulist1[0][$keys1[0]];
                            $this->process['water_capacity_table']['U2'] = $ulist1[1]['ullage'];
                            $this->process['water_capacity_table']['CV2'] = $ulist1[1][$keys1[0]];
                        }
                        $water_weight = round($this->suanfa($qiu1, $ulist1, $keys1, $xiukong, $chishui), 3);
                        $this->process['water_capacity_table']['process'] = self::$function_process;
                        self::$function_process = array();
                        $this->process['Water_volume'] = $water_weight;
                    }
            ...
            case 'c':
                ...
                if ($data['qufen'] == 'diliang') {
                     ...
                     * 水体积计量
                     */
                    $ulist = $this->downup($water_ullage, $shipmsg['zx_1'], $field, $data['cabinid']);
                    ...
                    //根据纵修与基准高度-空高的差值比较取小
                    $chazhi = round(($water_ullage - $altitudeheight), 3);
                    if ($chazhi > $zongxiu1) {
                        $zongxiu = $chazhi;
                    } elseif ($chazhi < $zongxiu1) {
                        $zongxiu = $zongxiu1;
                    ...
                    //得到修正空距 空距+纵倾修正值
                    $xiukong = $water_ullage - $zongxiu;
                    $this->process['Water_c_ullage'] = $xiukong;

                    //根据修正空距到容量表查询数据
                    $field1 = array('ullage', 'capacity');
                    $ulist2 = $this->downup($xiukong, $shipmsg['rongliang_1'], $field1, $data['cabinid']);
                    ...
                    //得到修正空距 空距+纵倾修正值
                    $water_xiukong = $water_ullage - $zongxiu;
                    $this->process['Water_c_ullage'] = $water_xiukong;

                    //根据修正空距到容量表查询数据
                    $field1 = array('ullage', 'capacity');
                    $ulist2 = $this->downup($water_xiukong, $shipmsg['rongliang_1'], $field1, $data['cabinid']);
                }
                //计算容量
                //当修正空高大于等于基准高度并且不计算底量的时候
                if ($data['quantity'] == '2' and $altitudeheight == $xiukong) {
                    ...
                    $this->process['water_capacity_table'] = array();
                    if (count($ulist2) == '1') {
                        $this->process['water_capacity_table']['U1'] = $ulist2[0]['ullage'];
                        $this->process['water_capacity_table']['CV1'] = $ulist2[0][$keys1[0]];
                        $this->process['water_capacity_table']['U2'] = $ulist2[0]['ullage'];
                        $this->process['water_capacity_table']['CV2'] = $ulist2[0][$keys1[0]];
                    } elseif (count($ulist2) == '2') {
                        $this->process['water_capacity_table']['U1'] = $ulist2[0]['ullage'];
                        $this->process['water_capacity_table']['CV1'] = $ulist2[0][$keys1[0]];
                        $this->process['water_capacity_table']['U2'] = $ulist2[1]['ullage'];
                        $this->process['water_capacity_table']['CV2'] = $ulist2[1][$keys1[0]];
                    }
                    $water_weight = round($this->suanfa($qiu1, $ulist2, $keys1, $water_xiukong, $chishui), 3);
                    $this->process['water_capacity_table']['process'] = self::$function_process;
                    self::$function_process = array();
                    $this->process['Water_volume'] = $water_weight;
...
//                echo jsonreturn($res);
        ...
        $allweight = $resultlist
            ->field("sum(cargo_weight) as sums")
            ->where($wheres1)
            ->select();
        //根据总标准容量*密度*(1-含水率)得到作业前/后总的货重
        $total = round($allweight[0]['sums'] * (1 - $sw), 3);
...
//            );
    ...
    public function get_cabins_weight($resultid)
    {
        ...
        // 根据计量ID获取密度，
        $msg = $this
            ->field('houdensity,qiandensity,houtotal,qiantotal,weight,shipid,add_weight,qiansw,housw')
        ...
        $list_qian_data = $resultlist
            ->alias('r')
            ->field('r.id,r.sounding,r.ullage,r.solt,r.temperature,r.cabinid,r.is_work,r.standardcapacity as cargo_weight,r.water_sounding,r.ob_temperature,r.ob_density,c.cabinname')
    ...
    public function adjust_cabin(array $data, $type = 'l')
    {
        ...
        $record_edit_data = array(
            ...
            'temperature' => $data['temperature'],
            'water_sounding' => $data['water_sounding'],
            'water_ullage' => round($record_data['altitudeheight'] - $data['water_sounding'], 3),
            'ob_temperature' => $data['ob_temperature'],
            'ob_density' => $data['ob_density'],
        );
        //保存新的值，捕获其中的异常
