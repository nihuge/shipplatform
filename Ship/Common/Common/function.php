    ...
    for ($i = 0; $i < $len; $i++) {
        ...
        if ($asc < 0x80) { // 0-127
            ...
            if (isset($pinyins[$chr])) {
                $rs .= 'first' === $ret_format ? $pinyins[$chr][0] : ($pinyins[$chr] . '_');
            } else {
                $rs .= $placeholder;
            }
        }
 ...
 */
function createQRcode($save_path, $qr_data = 'xiehzong', $qr_level = 'L', $qr_size = 2, $save_prefix = 'qrcode')
{
    ...
    if (isset($qr_data)) {
        ...
        //默认生成
        QRcode::png('xiezhong', $filename, $errorCorrectionLevel, $matrixPointSize, 2);
    }
 ...
 * @return string
 */
function reportCodeEncode($result_id, $user_id)
{
    $sign_str = "";
    $sign_str .= ($user_id << 4) . "&" . ($result_id << 3);
    return md5(md5($sign_str) . $result_id << 2);
}
 ...
 * @return string
 */
function shReportCodeEncode($result_id, $user_id)
{
    $sign_str = "";
    $sign_str .= ($user_id << 4) . "&&" . ($result_id << 3);
    return md5(md5($sign_str) . $result_id << 2);
