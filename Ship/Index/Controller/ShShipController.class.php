...
class ShShipController extends IndexBaseController
{
    ...
    public function addship()
    {
        if (I('post.firmid') and I('post.shipname') and I('post.lbp') != null
            ...
            if ($res == false) {
                //错误5，不能含有特殊字符
                echo ajaxReturn(array("state" => 5, 'message' => "不能含有特殊字符"));
            } else {
                $data['expire_time'] = strtotime(I('post.expire_time'));
                $res_s = $this->db->addship($data, 'APP');
                if ($res_s['code'] == 1) {
                    //添加船成功，将此船的操作和查看权限加给检验公司和管理员
                    echo ajaxReturn(array("state" => 1, 'message' => "新增船舶成功", 'shipid' => $res_s['content']['shipid']));
                } else {
                    echo ajaxReturn(array("state" => $res_s['code'], 'message' => $res_s['msg']));
                }
            }
        } else {
            //参数不正确，参数缺失	4
            echo ajaxReturn(array("state" => 4, 'message' => "不可以留空"));
        }
        echo ajaxReturn(array("state" => 2, 'message' => "表单出现错误"));
    }
            ...
            if (I('post.id')) {
                ...
                if ($count == 1) {
                    ...
                    if (!$this->db->create($data)) {
                        //数据库错误，错误3
                        echo ajaxReturn(array("state" => 3, 'message' => "数据库错误"));
//                        echo ajaxReturn(array("state" => 2, 'message' => $this->db->getError()));
                        ...
                        if ($res_count > 1 or $old_info['review'] == 2) {
                            ...
                            if ($diff_info['shipname'] !== null) {
                                ...
                                if ($name_count > 0 or $review_name_count > 0) {
                                    //船舶已存在   2014
                                    echo ajaxReturn(array("state" => 2014, 'message' => "船舶已存在"));
                                }
                            }
                            ...
                            if ($diff_info['cabinnum'] !== null) {
                                if ($diff_info['cabinnum'] >= $old_info['cabinnum']) {
                                    //不可以减少舱总数，2026
                                    echo ajaxReturn(array("state" => 2026, 'message' => "不可以减少舱总数"));
                                }
                            }
                            ...
                            if ($result !== false) {
                                //等待审核
                                echo ajaxReturn(array("state" => 200, 'message' => "请求提交成功，等待审核", 'review_id' => $result));
                            } else {
                                echo ajaxReturn(array("state" => 11, 'message' => "修改失败"));
                            }
                        } else {
                    ...
                    echo ajaxReturn(array("state" => 2002, 'message' => "船不存在"));
                }
            } else {
                //参数不正确，参数缺失	4
                echo ajaxReturn(array("state" => 4, 'message' => "表单不能留空"));

            }
        }
        echo ajaxReturn(array("state" => 2, 'message' => "失败，表单错误"));
    }
