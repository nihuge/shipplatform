...
class UserController extends IndexBaseController 
{
    ...
    public function editinfo()
    {
        if(I('post.username') and I('post.phone')){
			...
			if(!$this->db->create($data)){
				...
				if ($firmid == '') {
					echo ajaxReturn(array("state" => 2, 'message' => "请完善信息"));
				} else {
	        		// 修改用户信息
		            ...
		            if($resu !== false ){
						...
						echo ajaxReturn(array("state" => 1, 'message' => "修改个人信息成功"));
		            }else{
		            	echo ajaxReturn(array("state" => 2, 'message' => "修改个人信息失败"));
		            }					
				}
			}
        }else{
			echo ajaxReturn(array("state" => 2, 'message' => "表单不能有空值"));
        }
    }
            ...
            if($newpwd == $repeatpwd){
                ...
                if($msg != '')
                {
                    ...
                    if($pwdold == $msg['pwd'])
                    {
                        ...
                        if($res1 !== false)
                        {
							echo ajaxReturn(array("state" => 1, 'message' => "修改密码成功"));
                        }else {
							echo ajaxReturn(array("state" => 2, 'message' => "数据库操作错误"));
                        }
                    }else{
						echo ajaxReturn(array("state" => 2, 'message' => "原始密码不正确"));
                    }
                }else {
					echo ajaxReturn(array("state" => 2, 'message' => "该用户不存在"));
                }
            }else{
				echo ajaxReturn(array("state" => 2, 'message' => "新密码与确认密码不一致"));
            }
        }else{
			echo ajaxReturn(array("state" => 2, 'message' => "不能有空值"));
        }
    }
	...
	public function add()
	{
		if(I('post.title')){
			...
			echo ajaxReturn(array("state" => 2, 'message' => "账号为必填项"));
        }
	}
	...
	public function usermsg()
	{
	    ...
	    $string = "<div class='bar'>修改人员信息</div>
            ...
            <input type='hidden' name='userid' id='userid' value='".$data['id']."'>
                <li>
                    <label>用&nbsp;户&nbsp;名</label>
                    <p>
                    	<input type='text' name='username' placeholder='请输入用户名' class='i-box' id='nameuser1' maxlength='15' value='".$usermsg['username']."'>
                    </p>
                </li> 
                <li>
                    <label>联系电话</label>
                    <p><input type='text' name='phone' placeholder='请输入联系电话' class='i-box' id='phones1' maxlength='15' value='".$usermsg['phone']."'></p>
                </li>
            </ul>
            ...
            foreach ($shiplist as $key => $value) {
            	$string .= "<li>
                    <p>
                    	<label><input type='checkbox' name='operation_jur1' value='".$value['id']."' class='regular-checkbox' ".(in_array($value['id'], $operation_jur) ? 'checked' : '' ).">&nbsp;&nbsp;".$value['shipname']."</label>
                    </p>
                </li> ";
            }
            $string .= "</ul>
	...
	public function edit()
	{
		if(I('post.id')){
			...
			if(!$this->db->create($data)){
				//对data数据进行验证
				echo ajaxReturn(array("state" => $res['code'] , 'message' => $this->db->getError()));
			}else{
        		// 修改用户信息
	            $resu = $this->db->editData($map,$data);
	            if($resu !== false ){
	...
	public function configSearch()
	{
	    if (I('post.id')) {
	    	...
	    	if ($data !== false and !empty($data)) {
	    		...
	    		echo ajaxReturn(array("state" => 2, 'message' => "获取数据有误！"));
	    	}
	    }else{
	...
	public function searchconfig()
	{
		...
		if(!$this->db->create($data)){
            ...
            $resu = $this->db->editData($map,$data);
            if($resu !== false ){
            	echo ajaxReturn(array("state" => 1, 'message' => "修改用户查询条件成功"));
            }else{
            	echo ajaxReturn(array("state" => 2, 'message' => "修改用户查询条件失败！"));
            }
		}
	}
}